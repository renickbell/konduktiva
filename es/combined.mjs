...
export function updateTempFilesVariable (KonduktivaPackage){
...
export function getNextOnsets3 (ioiFunc, player, beat, limitBeat, output, e) {
    if (beat > limitBeat) {
        return output
    }
    else {
...
export function checkIfObjectCompatibleWithPlayer (item){
...
export function checkArrayForQuantizedMapAndConvert (arr){
    return arr.map(x => {
...
export function convertObjectToQuantizedMap (obj){
...
export function countCharacterBeforeIndex(str, index, char) {
    let countBefore = 0;
...
export function countCharacterAfterIndex(str, index, char) {
    let countAfter = 0;
...
export function checkIfVariableIsInString (string, varToCheckFor){
...
export function checkIfStringIsArrowFunction (string){
...
export function checkForAnonymousFunctionInString (string){
...
export function checkForNormalFunctionInString (string){
    let splitString = string.trim().split(' ').filter(str => str !== "");
...
export function checkIfStringIsAFunction (string){
    let splitString = string.trim().split(' ').filter(str => str !== "");
...
export function convertStringToFunction (string){
//     let functionResult;
//         try{
...
export function checkMidiProgramMap (keyspan, keys, values){
    if (checkAllItemsType(values, 'Array') === false){
...
export function checkModeMap (keyspan, keys, values, e){
    if (checkAllItemsType(values, 'string') === false){
...
//         console.log("scheduling " + player);
        ...
        let onsets = getNextOnsets3(ioiFunc,player, this.currentBeat(), this.currentBeat() + timeToBeats(this.currentTempo,this.lookahead),[], this);
...
export function addMapsTests (e, verbose = true){ 
...
export function defaultIOI (player, beat, e) {
...
export function changeRhythmPattern (env, players, rhythm, e) {
...
//Generate random configurations for the lsystem chord progression:
export function generateRandomLSystemConfiguration (pickedAlphabets = generateLsystemAlphabets()){
...
export function configureRhythmMapVariables (noteValueData, name, e){
...
export function createSongMap (noteValueData, name, e){
...
export function createChordMap (noteValueData, name, e){
...
export function createLegatoMap (noteValueData, name, e){
...
//     e.noteDurationMaps = {}
    e.rhythmPatterns = {'default': new RhythmPattern('default', 4, [0, 1, 2, 3], [true, true, true, true])}
...
export function addChordProgressionFromRomanNumeral(musicalEnvironment, progressionName, progressionKeyspan, progressionKeys, progression, key) {
...
export function sendMidiData(info, player, note, channel, e, b) {
    // add2Log(note)
    //add2Log('--------------------------------------------------------------------------')
...
export function filterpolyphony (e, b, player, info){
...
export function calculateFinalNoteValue (info, e){
...
export function findChannel (player, b, e){
...
export function sendNotesMidiInfo (playerName, b, e){
...
export function completePendingTask (playerName, b, e){
    let cb = e.currentBeat()
    let player = e.players[playerName]
...
export function getNoteInfoToSend(player, b, midiOutput, e) {
...
//Yiler export function start here:
...
export function generateCircleOfFifthsProgression (nChords, key) {
...
// }
// 
export function activateKeyboardFilter(keyboardName, e) {
...
export function findVariableWithMaxNumber(obj) {
...
export function compareSimilarityBetweenStrings (stringToCheck, stringToCompare){
...
export function getAllNoteOn (track){
    let notes = []
    track.forEach(x => {
        if (x.subtype == 'noteOn'){
...
export function findKeyOfChordProgression (midiNotes){
    let letters;
...
export function getDataFromMidiFile (midiFileData, e){
    let tempos = []
    let timeSignatures = []
    let endsOfTracks = []
    let name;
...
export function groupByAbsoluteTime(objects) {
  let result = [];
  let currentGroup = [];
...
export function midiToRootAndValue (midiNote){
...
export function splitMidiNote (midiNote, musicalKey){
...
export function checkIfTrackIncludesNoteOnOffMessages (midiTrack){
    return !midiTrack.every(x => {if (x.subtype === 'noteOn'){return false};return true})
}

export function findFileNameFromFilePath (filePath){
    let pathArray = filePath.split('/').join('\\').split('\\');
    return pathArray[pathArray.length - 1].split('.')[0]
}

export function getClosestSmallerDivisible(originalNumber, divisibleBy) {
...
export let simpleMelodyDataTemplate = {
        noteValuesKeyspan: 4,
...
export let defaultConfigurationObject = {
  channelKeyspan: 1,
  channelKeys: [0],
  channelValues: [1],
  velocity: A.buildArray(30, x => 90),
...
export function addDuplicatePlayersWithConfigObj (e, nOfPlayers, configObj, baseName, extraConfig){
...
export function emptyPlayerDataConfigObj (){
    return {
